...
(defvar basilisk-compilation-methods
  '(("Basic (No MPI)" . "qcc -Wall %s -o %s -lm")
  ...
  "List of compilation methods for Basilisk code. %d is for MPI processes.")
...
(defvar basilisk-run-methods
  ...
  "List of run methods for Basilisk code. %d is for MPI processes.")
...
(defun basilisk-compile-command-generator ()
  ...
  (let* ((src-file (or (buffer-file-name) (error "No file to compile; please save the buffer")))
    ...
    (if (string-match "MPI" compile-method)
        (let ((nproc (string-to-number (read-string "Number of MPI processes (1-200): " "1"))))
          (setq nproc (max 1 (min 200 nproc)))  ; Clamp between 1 and 200
          (if (string-match "MPI Portable Source" compile-method)
              (setq compile-cmd (format compile-template nproc source-name nproc source-name output-name))
            (setq compile-cmd (format compile-template nproc source-name output-name))))
      (if (string-match "Makefile" compile-method)
          (setq compile-cmd (format compile-template output-name))  ; Makefile uses base name
...
(defun basilisk-run ()
  ...
  (let* ((src-file (or (buffer-file-name) (error "No file to run; please save the buffer")))
         (output-name (basilisk-get-output-filename))  ; Base name for executable
    ...
    (if (string-match "MPI" run-method)
        (let ((nproc (string-to-number (read-string "Number of MPI processes (1-200): " "1"))))
          (setq nproc (max 1 (min 200 nproc)))  ; Clamp between 1 and 200
...
(defun basilisk-show-compilation-help ()
  ...
  (with-output-to-temp-buffer "*Basilisk Compilation Help*"
    ...
    (princ "=== Without MPI ===\n")
    (princ "- Basic: qcc -Wall code.c -o code -lm\n")
